...
class Toonstream : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        val url = if (page == 1) "$mainUrl/$path/" else "$mainUrl/$path/?paged=$page"
    ...
    // -----------------------------------------------------------
    // FIX 1: Handle both src and data-src (first iframe uses src)
    // FIX 2: Convert relative /embed/xxx URLs to absolute URLs
    // FIX 3: Use .Video iframe selector to skip fake player overlay
    // -----------------------------------------------------------
    override suspend fun loadLinks(
        ...
        document.select("#aa-options > div > iframe").forEach { iframe ->
            // FIX 1: First iframe uses `src`, rest use `data-src`
            val rawSrc = iframe.attr("data-src").ifEmpty { iframe.attr("src") }
            if (rawSrc.isEmpty()) return@forEach

            // FIX 2: Embed URLs are relative (/embed/xxx) — make absolute
            val serverlink = if (rawSrc.startsWith("http")) rawSrc else "$mainUrl$rawSrc"

            val truelink = try {
                // FIX 3: Select .Video iframe to skip the fake-player-overlay div
            ...
            if (truelink.isNotEmpty()) {
                loadExtractor(truelink, serverlink, subtitleCallback, callback)
            }
        }
        return true
    }
