...
class Toonstream : MainAPI() {
    ...
    // -----------------------------------------------------------
    // HOME PAGE TABS
    // Fresh Drop is handled separately (scraped from /home/).
    // All other entries are scraped from /<slug>/page/<n>/
    // -----------------------------------------------------------
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        // Fresh Drop -> scrape the home page widget (no pagination, ~18 items)
        ...
        // Other tabs -> standard paginated category scrape
    ...
    // Site does not expose /category/fresh-drop/ content, so we
    // grab the static widget on /home/ instead.
    // -----------------------------------------------------------
    private suspend fun fetchFreshDrop(): List<SearchResponse> {
        val document = app.get("$mainUrl/home/").document

        // Find the <h3>Fresh Drop</h3> section, walk up to the
        // nearest container that holds <article class="post dfx">
        ...
        return section.select("article.post.dfx").mapNotNull { el ->
            ...
            val season = el.selectFirst("span.post-ql")?.text()?.trim() // "Season 3"
            val episode = el.selectFirst("span.year")?.text()?.trim()    // "EP 11"

            val tvType = if (season != null || episode != null) TvType.TvSeries else TvType.Movie

            newMovieSearchResponse(title, href, tvType) {
                this.posterUrl = poster
                this.score = rating?.let { (it * 100).toInt() }   // 0..10 -> 0..1000
                this.otherInfo = listOfNotNull(season, episode).joinToString(" • ")
            }
        }
    }
    ...
    private fun Element.toSearchResult(): SearchResponse {
        ...
        val poster = if (posterUrlRaw.startsWith("http")) posterUrlRaw else "https:$posterUrlRaw"
        ...
        return if (tvtag == TvType.TvSeries) {
            ...
            document.select("div.aa-drp.choose-season > ul > li > a").forEach { info ->
                ...
                season.select("article").forEach {
                    ...
                    val seasonnumber = season.toString()
                        .substringAfter("<span class=\"num-epi\">")
                        .substringBefore("x")
                        .toIntOrNull()
                    episodes.add(
                        newEpisode(href) {
